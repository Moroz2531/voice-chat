...
Socket::Socket(int fd)
    : sfd_{fd}, refCount_{std::make_shared<std::atomic_size_t>(1)} {
#ifdef _WIN32
  if (::WSAStartup(MAKEWORD(2, 2), &wsaData_) != 0)
    throw std::runtime_error("WSAStartup");
#endif
...
Socket::Socket(const Socket& sock)
    : sfd_{sock.sfd_}, refCount_{sock.refCount_} {
  if (isValid()) {
#ifdef _WIN32
    if (::WSAStartup(MAKEWORD(2, 2), &wsaData_) != 0)
      throw std::runtime_error("WSAStartup");
...
netsize_t Socket::sendto(const char* buf,
  ...
  if (sendBytes == -1) {
    ...
    if (errno == ECONNRESET || errno == ECONNREFUSED || errno == EHOSTUNREACH ||
        errno == ENETUNREACH || errno == EPIPE || errno == ENETDOWN)
      return -1;
    throw std::runtime_error("sendto");
  }
#endif
...
netsize_t Socket::recvfrom(char* buf,
                           size_t count,
                           int flags,
                           sockaddr* addr,
                           socklen_t* addrLen) const {
...
netsize_t Socket::sendto(const float* buf,
                         size_t count,
                         int flags,
                         const sockaddr* addr,
                         socklen_t addrLen) const {
